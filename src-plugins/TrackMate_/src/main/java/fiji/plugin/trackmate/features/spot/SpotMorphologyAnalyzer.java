...
public class SpotMorphologyAnalyzer<T extends RealType<T>> extends IndependentSpotFeatureAnalyzer<T> {
	...
	public static void main(final String[] args) {
		...
		// Create blank image
		Image<UnsignedByteType> img = new ImageFactory<UnsignedByteType>(new UnsignedByteType(), new ArrayContainerFactory()).createImage(new int[] { 200, 200, 200 });
		final byte on = (byte) 255;
		...
		while (sc.hasNext()) {
			...
			term = r2 * cosphi * cosphi * sintheta * sintheta / a / a + r2 * sinphi * sinphi * sintheta * sintheta / b / b + r2 * costheta * costheta / c / c;
			if (term <= 1)
				sc.getType().set(on);
		}
		sc.close();
