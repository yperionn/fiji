...
public abstract class AbstractProvider {
	...
	/**
	 * The target keys. These names will be used as keys to access relevant
	 * classes. The list order determines how target classes are presented in
	 * the GUI.
	 */
	...
	/**
	 * The target classes pretty names. With the same order that for
	 * {@link #keys}.
	 */
	...
	/**
	 * Configure this provider for the target {@link SpotDetectorFactory}
	 * identified by the given key. If the key is not found in this provider's
	 * list, the provider state is not changed.
	 *
	 * @return true if the given key was found and the target detector was
	 *         changed.
	 */
	 ...
	 */
	protected boolean writeAttribute(final Map<String, Object> settings, final Element element, final String parameterKey, final Class<?> expectedClass) {
	...
	/**
	 * Stores the given mapping in a given JDon element, using attributes in a
	 * KEY="VALUE" fashion.
	 */
	 ...
	 * are added to the given map. If a value is found not to be a double, an
	 * error is returned.
	 *
	 * @return true if all values were found and mapped as doubles, false
	 *         otherwise and {@link #errorMessage} is updated.
	 */
	protected boolean unmarshallMap(final Element element, final Map<String, Double> map, final StringBuilder errorHolder) {
		boolean ok = true;
	...
	protected boolean readDoubleAttribute(final Element element, final Map<String, Object> settings, final String parameterKey, final StringBuilder errorHolder) {
	...
	protected boolean readIntegerAttribute(final Element element, final Map<String, Object> settings, final String parameterKey, final StringBuilder errorHolder) {
		final String str = element.getAttributeValue(parameterKey);
		if (null == str) {
			errorHolder.append("Attribute " + parameterKey + " could not be found in XML element.\n");
			return false;
	...
	protected boolean readBooleanAttribute(final Element element, final Map<String, Object> settings, final String parameterKey, final StringBuilder errorHolder) {
		final String str = element.getAttributeValue(parameterKey);
		if (null == str) {
			errorHolder.append("Attribute " + parameterKey + " could not be found in XML element.\n");
			return false;
	...
	/**
	 * @return a list of the detector informative texts available through this
	 *         provider.
	 */
