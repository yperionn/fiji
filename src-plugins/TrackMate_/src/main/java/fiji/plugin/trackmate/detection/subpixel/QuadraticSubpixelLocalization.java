/* #%L ImgLib: a general-purpose, multidimensional image processing library. %%
 * Copyright (C) 2009 - 2012 Stephan Preibisch, Stephan Saalfeld, Tobias
 * Pietzsch, Albert Cardona, Barry DeZonia, Curtis Rueden, Lee Kamentsky, Larry
 * Lindsey, Johannes Schindelin, Christian Dietz, Grant Harris, Jean-Yves
 * Tinevez, Steffen Jaensch, Mark Longair, Nick Perry, and Jan Funke. %% This
...
public class QuadraticSubpixelLocalization<T extends RealType<T>> implements Algorithm, Benchmark, MultiThreaded {
	...
	public boolean analyzePeak(final SubPixelLocalization<T> peak) {
		...
		do {
			...
			// compute the n-dimensional hessian matrix
			...
			for (int d = 0; d < numDimensions; ++d) {
				...
				if (Math.abs(subpixelLocation[d]) > threshold) {
					if (allowedToMoveInDim[d]) {
						...
						// set it to the position that is maximally away when
						// keeping the current base position
						// e.g. if (0.7) do 4 -> 4.5 (although it should be 4.7,
						// i.e. a new base position of 5)
						// or if (-0.9) do 4 -> 3.5 (although it should be 3.1,
						// i.e. a new base position of 3)
	...
	/**
	 * This method is called by the process method to allow to override how the
	 * matrix is inverted
	 */
	 ...
	 * @param maxtrixImage
	 *        - the input {@link Img}
	 * @return a {@link Matrix} or null if the {@link Img} is not one or
	 *         two-dimensional
	 */
	...
	/**
	 * Computes the pseudo-inverse of a matrix using Singular Value
	 * Decomposition
	 * 
	 * @param M
	 *        - the input {@link Matrix}
	 * @param threshold
	 *        - the threshold for inverting diagonal elements (suggested 0.001)
	 * @return the inverted {@link Matrix} or an approximation with lowest
	...
	final public static Matrix computePseudoInverseMatrix(final Matrix M, final double threshold) {
		...
		for (int j = 0; j < S.getRowDimension(); ++j) {
			...
			if (temp < threshold) // this is an inaccurate inverting of the
									// matrix
				temp = 1.0 / threshold;
			else
				temp = 1.0 / temp;
	...
	/**
	 * Computes the n-dimensional 1st derivative vector in 3x3x3...x3
	 * environment for a certain {@link Img} location defined by the position of
	 * the {@link RandomAccess}.
	 * 
	 * @param cursor
	 *        - the position for which to compute the Hessian Matrix
	 * @return Img<DoubleType> - the derivative, which is essentially a
	 *         one-dimensional {@link DoubleType} {@link Img} of size
	 *         [numDimensions]
	 */
	 ...
	 *        - the position for which to compute the Hessian Matrix
	 * @param Img
	 *        <DoubleType> - the derivative, which is essentially a
	 *        one-dimensional {@link DoubleType} {@link Img} of size
	 *        [numDimensions]
	 */
	final public static <T extends RealType<T>> void computeDerivativeVector(final RandomAccess<T> cursor, final Img<DoubleType> derivativeVector) {
		// instantiate a cursor to traverse over the derivative vector we want
		// to compute, the position defines the current dimension
	...
	/**
	 * Computes the n-dimensional Hessian Matrix in 3x3x3...x3 environment for a
	 * certain {@link Img} location defined by the position of the
	 * {@link RandomAccess}.
	 * 
	 * @param cursor
	 *        - the position for which to compute the Hessian Matrix
	 * @return Img<DoubleType> - the hessian matrix, which is essentially a
	 *         two-dimensional {@link DoubleType} {@link Img} of size
	 *         [numDimensions][numDimensions]
	 */
	 ...
	 *        - the position for which to compute the Hessian Matrix
	 * @param Img
	 *        <DoubleType> - the hessian matrix, which is essentially a
	 *        two-dimensional {@link DoubleType} {@link Img} of size
	 *        [numDimensions][numDimensions]
	 */
	final public static <T extends RealType<T>> void computeHessianMatrix(final RandomAccess<T> cursor, final Img<DoubleType> hessianMatrix) {
		...
		// instantiate a cursor to traverse over the hessian matrix we want to
		// compute, the position defines the current dimensions
		...
		while (hessianCursor.hasNext()) {
			...
			if (dimA == dimB) {
				...
				// we compute all elements above the diagonal (see below for
				// explanation)

				// other elements h(ab) are computed as a combination
				// of dimA (dimension a) and dimB (dimension b), i.e. we always
				// operate in a
