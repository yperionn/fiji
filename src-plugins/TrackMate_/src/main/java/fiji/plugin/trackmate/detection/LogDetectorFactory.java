...
public class LogDetectorFactory<T extends RealType<T> & NativeType<T>> implements SpotDetectorFactory<T> {
	...
	public SpotDetector<T> getDetector(final int frame) {
		final int targetChannel = (Integer) settings.get(KEY_TARGET_CHANNEL) - 1; // parameter
																					// is
																					// 1-based
		...
		detector.setNumThreads(1); // in TrackMate context, we use 1 thread per
									// detector but multiple detectors
		return detector;
	}
