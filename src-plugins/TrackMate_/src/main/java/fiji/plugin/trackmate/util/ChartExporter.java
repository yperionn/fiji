...
public class ChartExporter {
	 ...
	 * <p>
	 * We use a dirty hack for that: we first export to a physical SVG file,
	 * reload it, and use Apache FOP PDF transcoder to convert it to pdfs. It
	 * only works partially, for the text ends up in not being selectable in the
	 * pdf.
	 *
	 * @param chart
	 *            JFreeChart to export
	...
	public static void exportChartAsPDF(final JFreeChart chart, final Rectangle bounds, final File pdfFile) throws IOException, DocumentException {
		...
		final Graphics2D g2 = canvas.createGraphics(pageSize.getWidth(), pageSize.getHeight());
