...
public interface SpotAnalyzerFactory<T extends RealType<T> & NativeType<T>> extends FeatureAnalyzer {
	...
	/**
	 * @return a configured {@link SpotAnalyzer} ready to operate on the given
	 *         frame (0-based) and given channel (0-based). The target frame
	 *         image and the target spots are retrieved from the {@link Model}
