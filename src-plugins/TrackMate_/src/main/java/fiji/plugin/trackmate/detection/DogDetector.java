...
public class DogDetector<T extends RealType<T> & NativeType<T>> extends LogDetector<T> {
	...
	public boolean process() {
		...
		// And the out of bounds strategies for both types. It needs to be a
		// value-oobs, with a constant
		// value of 0; otherwise, we will miss maxima on the border of the
		// image.
		...
		for (int i = 0; i < sigma2.length; i++) {
			sigma1[i] = 2 / (1 + Math.sqrt(img.numDimensions())) * radius / calibration[i]; // in
																							// pixel
																							// units
		...
		if (doSubPixelLocalization && pruned_list.size() > 0) {
			...
			locator.setNumThreads(1); // Since the calls to a segmenter are
