	...
	public ArrayList<ArrayList<Spot>> splitTrackInBranches(Set<Spot> track) {
		...
		while (iterator.hasNext()) {
			...
			if (type == BRANCH_START) {
				// This can be a real branch start, unless we are iterating
				// backward in time.
				// Then this event should be a branch stop. We discriminate
				// between the 2 using
				// the previous spot: if it is connected to this one, then we
				// are moving backward
				...
				if (model.getTrackModel().containsEdge(spot, previousSpot)) {
					...
					currentParent = new ArrayList<Spot>(); // Create a new
															// branch for the
															// next spot
				} else {
					// branch start
		...
		// In the general case, there might be empty branches. We prune them
		// here
