...
public class TrackSplitter {
	...
	public ArrayList<ArrayList<Spot>> splitTrackInBranches(final Set<Spot> track) {

		final SortedSet<Spot> sortedTrack = new TreeSet<Spot>(Spot.timeComparator);
		sortedTrack.addAll(track);
		final Spot first = sortedTrack.first();

		final ArrayList<ArrayList<Spot>> branches = new ArrayList<ArrayList<Spot>>();
		...
		while (iterator.hasNext()) {
			...
			final int type = getVertexType(model, spot);
		...
		final ArrayList<ArrayList<Spot>> prunedBranches = new ArrayList<ArrayList<Spot>>();
		for (final ArrayList<Spot> branch : branches) {
	...
	public static final int getVertexType(final Model model, final Spot spot) {
		...
		final int nConnections = edges.size();

		if (nConnections == 0)
			return LONE_VERTEX;

		final int t0 = spot.getFeature(Spot.FRAME).intValue();

		if (nConnections == 1) {
			final DefaultWeightedEdge edge = edges.iterator().next();
			Spot other = model.getTrackModel().getEdgeSource(edge);
			if (other == spot)
				other = model.getTrackModel().getEdgeTarget(edge);
			final int t1 = other.getFeature(Spot.FRAME).intValue();
			if (t1 > t0)
				return BRANCH_START;
			else
		...
		if (nConnections == 2) {
			final Iterator<DefaultWeightedEdge> it = edges.iterator();
			final DefaultWeightedEdge edge1 = it.next();
			Spot other1 = model.getTrackModel().getEdgeSource(edge1);
			if (other1 == spot)
				other1 = model.getTrackModel().getEdgeTarget(edge1);
			final int t1 = other1.getFeature(Spot.FRAME).intValue();
			final DefaultWeightedEdge edge2 = it.next();
			Spot other2 = model.getTrackModel().getEdgeSource(edge2);
			if (other2 == spot)
				other2 = model.getTrackModel().getEdgeTarget(edge2);
			final int t2 = other2.getFeature(Spot.FRAME).intValue();
		...
		for (final DefaultWeightedEdge edge : edges) {
			...
			if (other == spot)
				other = model.getTrackModel().getEdgeTarget(edge);
			final double t = other.getFeature(Spot.POSITION_T);
			if (t > t0)
				after++;
			if (t < t0)
				before++;
